...
const pendingTasks  = new Map();   // uuid -> {title,deadline,description}
...
client.once('ready', async () => {
  ...
  const msg  = await buildMorningMessage(TARGET_USER_ID, 420, 15, 390); // 仮データ
...
const sendNews = label => async () => {
  ...
  const user   = await client.users.fetch(TARGET_USER_ID);
  const blocks = await getFormattedNews(label);           // 配列
...
/* ────────── cron スケジュール ────────── */
cron.schedule('1 6  * * *', { timezone:'Asia/Tokyo' }, sendNews('朝'));
cron.schedule('0 12 * * *', { timezone:'Asia/Tokyo' }, sendNews('昼'));
cron.schedule('0 20 * * *', { timezone:'Asia/Tokyo' }, sendNews('夜'));

cron.schedule('0 22 * * *', { timezone:'Asia/Tokyo' }, async () => {
  const user  = await client.users.fetch(TARGET_USER_ID);
...
async function handleRouteButton(interaction){
  ...
  for(const a of aList){
    ...
    if(b){ routes.push(`${isGo?'博多南':'福工大前'} ${formatTime(a)} → 博多 ${formatTime(b)}`); if(routes.length>=2)break; }
  }
  await interaction.reply({ content: routes.length?
    `【${isGo?'通学':'帰宅'}案内】\n`+routes.join('\n'): '適切なルートが見つかりません', ephemeral:true });
}
...
client.on(Events.InteractionCreate, async interaction => {
  try{
    ...
    if(interaction.isButton() && interaction.customId==='study_end'){
      ...
      return interaction.reply({ content:`勉強 ${dur} 分\nカテゴリを選択：`, components:[row],ephemeral:true });
    }

    /* カテゴリ確定 → Notion 保存 */
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('study_cat')){
      const cat=interaction.values[0]; const sess=studySessions.get(uid);
      if(!sess) return;
    ...
    if(interaction.isStringSelectMenu() && interaction.customId.startsWith('task_type')){
      ...
      const task=pendingTasks.get(uuid); if(!task) return;
...
/* ────────── Slash Command 登録 ────────── */
const commands=[ new SlashCommandBuilder().setName('task').setDescription('📝 タスク追加') ].map(c=>c.toJSON());
